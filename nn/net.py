...
class TwoLayerNet:
    ...
    def __init__(
        ...
        ######################################################################
        self.modules["linear1"] = Linear(input_dim,hidden_dim)
        self.modules["relu"] = ReLU()
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        scores = self.modules["linear1"].forward(X)
        scores = self.modules["relu"].forward(scores)
        scores = self.modules["linear2"].forward(scores)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
        if y is None:
        ...
        ######################################################################
        loss,d = self.modules["softmax"].forward(scores,y)
        d = self.modules["linear2"].backward(d)
        d = self.modules["relu"].backward(d)
        d = self.modules["linear1"].backward(d)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
        return loss
...
class FCNet:
    ...
    def __init__(
        ...
        ######################################################################
        self.modules["linear0"] = Linear(input_dim,hidden_dims[0])
        for i in range(self.num_layers-1):
            self.modules["relu" + str(i)] = ReLU()
            if i == (self.num_layers-2) :
                self.modules["linear" + str(i+1)] = Linear(hidden_dims[i],num_classes)
            else :
                self.modules["linear" + str(i+1)] = Linear(hidden_dims[i],hidden_dims[i+1])
        self.modules["softmax"] = SoftmaxCELoss()
    ...
    def loss(self, X, y=None):
        ...
        # HINT: 임의의 레이어를 처리하기 위해 for loop를 사용해야 함.
        ######################################################################
        scores = self.modules["linear0"].forward(X)
        for i in range(self.num_layers-1):
            scores = self.modules["relu" + str(i)].forward(scores)
            scores = self.modules["linear" + str(i+1)].forward(scores)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
        if y is None:
        ...
        ######################################################################
        loss,d = self.modules["softmax"].forward(scores,y)
        d = self.modules["linear" + str(self.num_layers-1)].backward(d)
        for i in range(self.num_layers-1):
            d = self.modules["relu" + str(self.num_layers-2-i)].backward(d)
            d = self.modules["linear" + str(self.num_layers-2-i)].backward(d)
        ######################################################################
        #                          END OF YOUR CODE                          #
        ######################################################################
        return loss
